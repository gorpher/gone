...
func Exec(opt *Options) (*CmdOutput, error) {
	logger.Debug(logSender, "Running command: %s %s", opt.Command, strings.Join(opt.CliArgs, " "))
	...
	logger.Debug(logSender, "Running command: %s %s at %s", opt.Command, strings.Join(opt.CliArgs, " "), opt.WorkingDir)
	logger.Debug(logSender, "Running command: %s env is %v", opt.Command, cmd.Env)
	...
	case <-opt.CancelCtx.Done():
		if IsWindows() && cmd.Process.Kill() != nil {
			logger.Error(logSender, "force kill app failed")
			err = ErrorForceKill
			return out, err
		}
		if cmd.Process.Signal(os.Interrupt) != nil {
			logger.Error(logSender, "force kill app failed, in %s", runtime.GOOS)
			err = ErrorForceKill
		}
	}
	return out, err
