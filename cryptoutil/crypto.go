...
func GenerateBase64Key(secretLength core.SecretKeyLengthType,
	...
	if secretLength == core.RSA {
		priKey, err = rsa.GenerateKey(rand.Reader, 2048)
	}

	if secretFormat == core.PKCS1 {
		// 生成公匙
...
func GenerateSSHKey(bits RSABit) (pkBytes []byte, pbkBytes []byte, err error) {
	...
	// golang.org/x/cryptoutil/ssh/keys.go:1129
...
func DecodePemHexBase64(data []byte) ([]byte, error) {
	...
	decoded, err = core.Base64StdDecode(data)
	if err == nil {
		return decoded, nil
	}
	decoded, err = core.Base64RawStdDecode(data)
	if err == nil {
		return decoded, nil
	}
	decoded, err = core.Base64URLDecode(data)
	if err == nil {
		return decoded, nil
	}
	return core.Base64RawURLDecode(data)
}
